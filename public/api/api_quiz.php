...
if ($action == 'list_quizzes') {
    try {
        $stmt = $pdo->query("SELECT q.*, c.name as chapter_name, s.name as subject_name, 
                            (SELECT SUM(marks) FROM questions WHERE quiz_id = q.id) as total_marks
                            FROM quizzes q 
                            JOIN chapters c ON q.chapter_id = c.id 
                            JOIN subjects s ON c.subject_id = s.id 
                            WHERE q.date_of_quiz >= CURDATE() 
...
if ($action == 'submit_attempt') {
    ...
    try {
        ...
        // Check and resolve bet
        $stmt = $pdo->prepare("SELECT * FROM bets WHERE user_id = ? AND quiz_id = ? AND status = 'pending'");
        $stmt->execute([$user['id'], $quiz_id]);
        $bet = $stmt->fetch();
        
        $bet_result = null;
        if ($bet) {
            $won = $total >= $bet['target_score'];
            $payout = $won ? $bet['bet_amount'] * 2 : 0;
            $status = $won ? 'won' : 'lost';
            
            // Update bet
            $stmt = $pdo->prepare("UPDATE bets SET actual_score = ?, payout = ?, status = ? WHERE id = ?");
            $stmt->execute([$total, $payout, $status, $bet['id']]);
            
            // Add payout to wallet if won
            if ($won) {
                $stmt = $pdo->prepare("UPDATE users SET wallet_balance = wallet_balance + ? WHERE id = ?");
                $stmt->execute([$payout, $user['id']]);
                $_SESSION['user']['wallet_balance'] += $payout;
            }
            
            $bet_result = ['won' => $won, 'payout' => $payout, 'target' => $bet['target_score']];
        }
        
        echo json_encode(['success'=>true,'score'=>$total,'attempt_id'=>$pdo->lastInsertId(),'bet_result'=>$bet_result]);
    } catch (Exception $e) {
        echo json_encode(['success'=>false,'message'=>'Database error']);
    }
    exit;
