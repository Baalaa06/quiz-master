...
<body>
    ...
    function submitQuiz() {
        ...
        .then(response => {
            if (response.success) {
                let message = `Quiz submitted successfully! Your score: ${response.score}`;
                if (response.bet_result) {
                    const bet = response.bet_result;
                    message += `\n\nBet Result: ${bet.won ? 'WON' : 'LOST'}`;
                    message += `\nTarget: ${bet.target}, Actual: ${response.score}`;
                    if (bet.won) {
                        message += `\nPayout: ₹${bet.payout}`;
                    }
                }
